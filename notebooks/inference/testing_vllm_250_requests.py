...
async def main_async():
    ...
    reviews = df["review_text"]  # Все 25к примеров
    ...
    async def send_request(session, review, index):
        # Ожидаем пока семафор разрешит выполнение
        async with semaphore:
            try:
                request_start = time.time()
                payload = {
                    "model": "JosephThePatrician/qwen3_0.6b-reviews-fine-tune-v2",
                    "messages": [
                ...
                async with session.post(
                    ...
                    results.append({
                        ...
                        "response": response_data.get('choices', [{}])[0].get('message', {}).get('content') if response.status == 200 else None
                    })
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [send_request(session, review, i) for i, review in enumerate(reviews)]
        # Выполняем задачи с ограничением через семафор
